-- testing some output possibilities
...
local function save_hash_to_file(hash_value, filename)
  ...
  file:write(hash_value) -- Write just the hash
  file:close()
  print("Hash saved to " .. filename)
end

local function process_api_output(api_output, filename)
  ...
  -- Get the first sha256 hash and save it
  if #data.result > 0 then
    local first_hash = data.result[1].sha256
    save_hash_to_file(first_hash, filename)
  end
...
process_api_output(api_output, "first_hash.txt")
